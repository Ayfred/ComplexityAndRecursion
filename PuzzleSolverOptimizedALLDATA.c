...
void chooseDate(int m, int j, int s);
void write_csv_file(char* filename, int num_rows, double* column1, double* column2, double* column3, double* column4, double* column5);

//
#define TOTALVALUES 12*7*31
double months[TOTALVALUES];
double day[TOTALVALUES];
double week[TOTALVALUES];
double totalSolutions[TOTALVALUES];
double SecondGPU[TOTALVALUES];
...
int main(){
    ...
    for(int m = 1; m <= 12; m++){
        for(int j = 1; j <= 31; j++){
            for(int s = 1; s <= 7; s++){
                ...
                day[m*j*s] = j;
                week[m*j*s] = s;
                totalSolutions[m*j*s] = total;
                SecondGPU[m*j*s] = time_spent;

                int board[ROWS][COLUMNS] = {0}; //le plateau de jeu est réinitialisé
                board[0][6] = 11;
                board[1][6] = 11;
                board[7][0] = 11;
                board[7][1] = 11;
    ...
    write_csv_file("data.csv", TOTALVALUES, months, day, week, totalSolutions, SecondGPU);
...
void write_csv_file(char* filename, int num_rows, double* column1, double* column2, double* column3, double* column4, double* column5) {
    FILE* fp;
    fp = fopen(filename, "w");
    fprintf(fp, "Mois, Quantième, Jour, #Solutions, Secondes GPU\n");
